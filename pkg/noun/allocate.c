...
/* _u3a_bid_cmp(): qsort comparator for c3_d blob IDs.
*/
static int
_u3a_bid_cmp(const void* a_v, const void* b_v)
{
...
**
**   Walks the page directory of u3R (must be the home road) scanning all
**   chunk-allocated hunk pages.  For each allocated chunk large enough to
**   hold a u3a_atom, checks whether the len_w field has u3a_blob_flag set.
**   If so, extracts mug_h and seq_w and packs them into a c3_d blob ID
**   (mug_h in high 32 bits, seq_w in low 32 bits).
**
**   Returns a malloc'd sorted array of blob IDs; sets [*out_z] to the count.
**   Caller must c3_free() the array.
**
**   This is an O(heap_pages) scan, much faster than u3a_walk_fore on the
**   full noun tree.  Used by the epoch-chop GC to rebuild the live blob set
**   without touching the loom allocator (safe when loom may be undersized).
*/
c3_d*
u3a_find_bobs(c3_z* out_z)
{
  //  word offset of len_w within u3a_atom (= c3_wiseof(u3a_noun))
  //
  c3_w len_off_w = c3_wiseof(u3a_noun);

  //  exact chunk size (in words) of a bob atom: header + 1 data word.
  //  bob atoms always occupy this exact size class and no other.
  //
  c3_w bob_wor_w = c3_wiseof(u3a_atom) + 1;

  u3p(u3a_crag)* dir_u = u3to(u3p(u3a_crag), HEAP.pag_p);

  //  cap scan to pages actually mapped in the current loom allocation
  //  (HEAP.len_w reflects the loom size at snapshot time, which may be
  //  larger than the loom mapped by the current process, e.g. when
  //  `urbit chop` runs with the default --loom 32 on a pier created with
  //  a larger loom)
  //
  c3_w map_w = (c3_w)(u3C.wor_i >> u3a_page);
  c3_w lim_w = ( HEAP.len_w < map_w ) ? HEAP.len_w : map_w;

  if ( HEAP.len_w > map_w ) {
    fprintf(stderr, "warn: loom smaller than snapshot (%" PRIc3_w " > %" PRIc3_w " pages); "
                    "blob GC may miss blobs — run with matching --loom\r\n",
            HEAP.len_w, map_w);
  }

  //  dynamic C-heap array; grows as needed
  //
  c3_z  cap_z = 64;
  c3_z  len_z = 0;
  c3_d* ids_d = c3_malloc(cap_z * sizeof(c3_d));

  for ( c3_w pag_w = 0; pag_w < lim_w; pag_w++ ) {
    u3_post dir_p = dir_u[pag_w];

    //  skip free and large (multi-page) allocations — bob atoms are small
    //
    if ( dir_p <= u3a_rest_pg ) {
      continue;
    }

    //  hunk page: dir_p is the crag post
    //
    u3a_crag* pag_u = u3to(u3a_crag, dir_p);
    c3_s      log_s = pag_u->log_s;

    //  bob atoms are always exactly bob_wor_w = (c3_wiseof(u3a_atom) + 1) words.
    //  that falls into the unique size class whose chunk size equals bob_wor_w.
    //  skip all other size classes to eliminate false positives from cells,
    //  HAMT nodes, or any other allocation whose word-2 happens to equal
    //  (u3a_blob_flag | 1) by coincidence.
    //
    if ( (c3_w)1 << log_s != bob_wor_w ) {
      continue;
    }

    c3_s      tot_s = u3a_Hunk[log_s - u3a_min_log].tot_s;
    c3_s      hun_s = u3a_Hunk[log_s - u3a_min_log].hun_s;

    u3_post pag_p = page_to_post(pag_w);

    for ( c3_s pos_s = hun_s; pos_s < tot_s; pos_s++ ) {
      //  bitmap: bit SET means FREE, bit CLEAR means ALLOCATED
      //
      c3_w blk_w = pos_s >> u3a_word_bits_log;
      c3_w bit_w = (c3_w)1 << (pos_s & (u3a_word_bits - 1));

      if ( pag_u->map_w[blk_w] & bit_w ) {
        continue;   //  free chunk
      }

      //  allocated chunk — probe len_w at the expected offset
      //
      u3_post hun_p = pag_p + ((c3_w)pos_s << log_s);
      c3_w*   raw_w = u3to(c3_w, hun_p);

      //  check len_w for bob atom: must be exactly (u3a_blob_flag | 1)
      //  (1 data word + flag).  This exact-match avoids false positives
      //  from cells whose hed/tel fields happen to have the high bit set.
      //
      if ( raw_w[len_off_w] != (u3a_blob_flag | (c3_w)1) ) {
        continue;
      }

      //  it's a bob atom: extract mug and seq
      //
      u3a_atom* atm_u = (u3a_atom*)raw_w;
      c3_h      mug_h = atm_u->mug_h;
      c3_w      seq_w = (c3_w)atm_u->buf_w[0];
      ...
      if ( len_z == cap_z ) {
        cap_z *= 2;
        ids_d = c3_realloc(ids_d, cap_z * sizeof(c3_d));
      }
      ids_d[len_z++] = bid_d;
    }
  }
  ...
  //
  if ( len_z > 1 ) {
    qsort(ids_d, len_z, sizeof(c3_d), _u3a_bid_cmp);
  }
